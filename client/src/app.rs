...
pub async fn run() {
    ...
    let window = Arc::new(
        WindowBuilder::new()
            .with_title("initial — AI Character & Animation Studio")
            .build(&event_loop)
                    ...
                    WindowEvent::RedrawRequested => {
...

                        {
                            ...
                            for (giz_verts, giz_idxs, giz_mat, giz_col, giz_met, giz_rough) in collect_gizmo_data(&editor.scene) {
                                static_renderer.add_mesh(giz_verts, giz_idxs, giz_mat, giz_col, giz_met, giz_rough);
                            }
                            static_renderer.flush(&device);
