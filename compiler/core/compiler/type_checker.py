...
class TypeInferencer(ExprTransformer):
    ...
    def visit_Vec3(self, expr: Vec3) -> Expr:
        ...
        for c, name in zip([x, y, z], ["x", "y", "z"]):
            if c.value_type != ValueType.SCALAR:
                raise TypeError(
                    f"Vec3 component {name} must be SCALAR, got {c.value_type}",
                    span=expr.span,
                    node_type="Vec3",
                )
    ...
    def _visit_binary_op(self, expr: BinaryOp, node_class: type) -> Expr:
        ...
        if left.value_type != right.value_type:
            ...
            if node_class == Mul and (
                (
                    left.value_type == ValueType.SCALAR
                    and right.value_type == ValueType.VEC3
                )
                or (
                    left.value_type == ValueType.VEC3
                    and right.value_type == ValueType.SCALAR
                )
            ):
                return node_class(
                    left=left, right=right, span=expr.span, value_type=ValueType.VEC3
                )

            raise TypeError(
                f"Cannot perform {node_class.__name__} on {left.value_type} and {right.value_type}",
                span=expr.span,
        ...
        return node_class(
            left=left, right=right, span=expr.span, value_type=left.value_type
        )
    ...
    def visit_Pow(self, expr: Pow) -> Expr:
        ...
        if left.value_type != ValueType.SCALAR or right.value_type != ValueType.SCALAR:
            raise TypeError(
                f"Pow requires SCALAR operands, got {left.value_type} and {right.value_type}",
                span=expr.span,
                node_type="Pow",
            )
    ...
    def _visit_unary_op(self, expr: UnaryOp, node_class: type) -> Expr:
        ...
        if inner.value_type != ValueType.SCALAR:
            raise TypeError(
                f"{node_class.__name__}() requires SCALAR input, got {inner.value_type}",
                span=expr.span,
                node_type=node_class.__name__,
            )
